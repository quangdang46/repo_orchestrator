...
**One pipeline, three verbs, and every step of it used to be a command.** `ro commit` and `ro push` are the same per-repo pipeline with a different stopping point — `commit` stops after the engine, `push` continues to the push, `ro ship` is both in one word. An earlier revision split this into `ro checkpoint`, `ro pr`, and `ro conflict`, and the split was wrong in a specific way rather than merely excessive: the first verb always had to know whether the others were coming, and *"I have work and I want it off my machine now"* is all of it at once. `ro sync` is a **different pipeline** (§6.3.1) that must not touch the index or spawn anything. See §6.3 for the full step table and the three-entry-point comparison.

**Conflict resolution is a stage inside `ro push`, gated behind `--resolve`.** `fetch → rebase → conflict? → Claude/Codex resolve → continue → push`. There is no `ro conflict` verb: it was proposed, and an interactive command over twenty repos is not a command — the same rule that keeps `ro commit` from cloning mid-transaction. Two invariants live in the implementation. `--resolve` **verifies the index is free of unmerged entries before** running `rebase --continue` (running it with conflicts still present produces a second, more confusing failure and the user learns to avoid the flag), and the engine that resolves **edits files and stages them, then stops** — ro runs `--continue` and the push. It is off by default: a model that resolves a conflict nobody had is worse than a stop, and the overwhelmingly common cause of a rejected push is a stale branch, which is three git commands that need no model.

**`ro push` refuses on a protected branch rather than inventing a branch name.** On `main`/`master` it errors with the fix: `git checkout -b feat/x` first. The old behaviour — silently create `ro/wip/<slug>-<run>` — existed only to give a pull request something to point at, and with PRs cut the reason is gone. Naming a branch has consequences (what the teammate fetches, what appears in `git branch` next month) and the user is standing right there. `--onto <BRANCH>` covers the rare case of pushing somewhere other than the default branch.

**New:** `crates/ro/src/ship/{mod,orchestrator,emit,summary}.rs` per §6.3 — one module serving all three entry points, parameterised by how far the pipeline runs. `crates/ro-sync/src/targets.rs` — `resolve_multi_repo_targets` lifted from `main.rs:554-611` with the `has:` bool-expression bug fixed, the invalid-glob-becomes-`*` bug fixed, `archived = 0 AND disabled = 0` added, and a real `repo_tags` reader. An empty selector resolves to **every row in the registry**, never a filesystem scan.

**`collect_git_dirs` must be made public.** It is a **private** `fn collect_git_dirs(dir, depth, out)` at `ro-sync/src/prune.rs:163`, with `const MAX_DEPTH: usize = 4` at `:164`, and `ro-sync/src/lib.rs` has no re-export. The only public entry point is `find_orphans`, which is coupled to SQLite and returns `Vec<Orphan>` after the tracked-path comparison — routing anything read-only through it routes the call through the database *and* through the exact code Phase 1 is repairing. Make the walker `pub` (or move it to `ro-git::discover_dirs`) and turn `MAX_DEPTH` into a parameter.

**CLI:** register the ten commands of §3 — `init`, `add`, `list`, `status`, `sync`, `commit`, `push`, `ship`, `doctor`, `config`, plus `schema`. There is **no `ro repos` namespace** (regroup reverted: at this surface size a namespace disambiguates nothing, and it broke the four most-used verbs). Add clap `alias`/`visible_alias` for the **removed** flat names — `ro sweep commit-sweep` → `ro sync`, `ro status` → `ro status` staying — printing a deprecation line and naming the removal version; there are currently zero aliases anywhere in the clap tree, so nothing is in the way. Consolidate the two duplicate `OutputFormat` enums (`main.rs:22` had 3 variants, `ro-output/src/lib.rs:25` had 4, and the CLI used neither) into one. Delete `--quiet` and `--verbose` after their deprecation release — `--verbose` is read nowhere, and `--quiet`'s only real read is inside the `ro import` branch Phase 1 deletes.

**Exit codes:** the `FatalError` type, the 64 remap, the per-command table (§6.8). Note that `2` currently means "clap usage error" and the fleet table wants `2 = all failed` — the two must not collide.

**Run history is cut, and the stdout summary is what replaces it.** `ro_jobs::open_run` / `append_event` / `finalize_run` have **zero production callers** today — they exist, they are tested, and nothing calls them, which is exactly the kind of thing that looks like a feature and is not one. With no `ro run` verb there is nothing to read the rows with, and a database nobody queries is not an audit trail. So: **do not wire them.** What replaces the trail is the per-repo summary on stdout (§6.11) — one line per repo naming branch, engine, the account the credential resolved to, commit count, and result — plus the NDJSON form under `--format ndjson`. That answers *"what did ro do to this repo"*; it does **not** answer *"what did ro do last Tuesday"*, and that gap is named in §1 rather than hidden behind a table nobody queries.

**Fix the sync FK bug while there.** `sync_all` mints `run_id = Uuid::new_v4()` at `ro-sync/src/sync.rs:306` and never inserts a `runs` row, but `sync_results.run_id` is `NOT NULL REFERENCES runs(id)` and `ro_state::open_db` sets `PRAGMA foreign_keys=ON`. The insert fails and is swallowed by `let _ = conn.execute(…)` at `sync.rs:326`. So `sync_results` is **permanently empty**, `ro status`'s `last_synced_at` is permanently NULL, and the health scorer's failed-sync penalty is permanently zero. **A failed pull is recorded as success** — the pull branch never checks `outcome.result.ok()` (the clone branch immediately above it does), and `PullOutcome.conflict` is computed and then discarded, so a conflicted `--rebase` pull lands in `sync_results` as `updated` / `success` / `error: None`. Add the `archived`/`disabled` filter to `sync_all`, wire `-j` and `--timeout` to real executors, and delete `--resume` (there is no daemon to resume into).
...
**Docs:** full `README.md` and `FEATURES.md` rewrite against reality. Fix at minimum: the `ro review plan --format json` claim (no such flag; `repo` was a required positional), the `ro fork sync` flags that never existed, the `ro robot-docs all` topic that never existed, the run-recording claim, and the `--parallel`/`--resume`/`--verbose` no-ops. **Preserve `README.md:415-417`** (the no-outside-contributions policy) and the `ro-illustration.webp` reference. `install.ps1:138` claims config lives at `%LOCALAPPDATA%\ro\config.toml`; `ro init` actually writes it to `%APPDATA%` (Roaming) via `dirs::config_dir()` and `state.db` to `%LOCALAPPDATA%`. `install.sh:234-236` warns "set `RO_FORCE=0` to refuse" when `0` is already the default and the install is unconditional.

**Depends on:** Phase 4.
**Verified by:** the §8 suite; a `ro sync --dry-run` printing a per-repo summary naming the account each credential resolved to; a live 3-repo fleet run with one deliberately failing repo producing exit 1; and `ro help` listing exactly the ten commands.
**Done looks like:** `init`, `add`, `list`, `status`, `sync`, `commit`, `push`, `ship`, `doctor`, `config`, `schema` — no review, no `ro health` command, no fork, no sweep, no import, no self-update, no `ro repos` namespace, no pull requests, no toon, and every documented claim true. The health *scorer* survives and stays filterable; only the command and the display are gone. Four modules, five migrations, ten commands in three tiers.
